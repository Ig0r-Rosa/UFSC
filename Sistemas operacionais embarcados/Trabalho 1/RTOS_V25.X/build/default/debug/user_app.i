...
TASK tarefa_acelerador() {
    ...
    while (1) {
        ...
        posicao_pedal = soma / 3;


        LATDbits.LATD2 = (posicao_pedal > 512) ? 1 : 0;
...
TASK tarefa_injecao_eletronica() {
    ...
    while(1) {
        mutex_lock(&injecao_mutex);
        uint16_t tempo = freio_acionado ? 0 : tempo_injecao;
        mutex_unlock(&injecao_mutex);

        pwm_set_duty(1, tempo > 255 ? 255 : tempo);
        delay(5);
    }
}
...
void user_config() {
    ...
    __asm("global _tarefa_acelerador, _tarefa_controle_central, _tarefa_injecao_eletronica, _tarefa_controle_estabilidade");
}
