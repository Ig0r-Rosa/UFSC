"""Exporta todos os gráficos do Trabalho 3, agrupados por questão."""
...
def plot_degrau(T, nome: str, titulo: str, tmax: float = 8, ref: float = 1.0) -> None:
    ...
    if info["Overshoot"] > 0.1:
        ax.plot(info["PeakTime"], info["Peak"], "ro", ms=7,
                label=f"Pico: {info['Overshoot']:.1f}%")
...
def plot_polos(T, nome: str, titulo: str, zeta_ref: float | None = None) -> None:
    polos = ct.poles(T)
    zeros = ct.zeros(T)
    fig, ax = plt.subplots(figsize=(6.5, 4.5))
    ax.scatter(np.real(polos), np.imag(polos), marker="x", s=80,
               color="crimson", label="Polos", zorder=3)
    if len(zeros):
        ax.scatter(np.real(zeros), np.imag(zeros), marker="o", s=60,
    ...
    if zeta_ref is not None:
        ...
        if len(cx):
            ...
            ax.plot([0, -2 * sigma], [0, 2 * wd], "g--", alpha=0.6,
...
def plot_polos_ab_fech(T_ol, T_cl, nome: str, titulo: str, zeta_ref: float | None = None) -> None:
...
def plot_lgr(L, nome: str, titulo: str) -> None:
...
plot_degrau(T1, "q1_degrau.png", "Q1 — Resposta ao degrau", 5)
...
plot_degrau(T2, "q2_degrau.png", r"Q2 — Resposta ao degrau ($\zeta=0{,}7$)", 3)
...
plot_degrau(T3, "q3_degrau.png", r"Q3 — Resposta ao degrau ($\zeta=0{,}5$)", 1.5)
...
# --- Q4 ---
G4 = 5 / (1 + 0.5 * s)
C4 = (9.79 * s + 160.38) / (s + 8.1)
L4, T4 = C4 * G4, ct.feedback(C4 * G4, 1)
info4 = ct.step_info(T4)
t4 = np.linspace(0, 0.8, 2000)
tout4, yout4 = ct.step_response(T4, t4)
yss4 = info4["SteadyStateValue"]
fig, ax = plt.subplots(figsize=(7, 4))
ax.plot(tout4, yout4, "b", lw=2)
ax.axhline(yss4, color="k", ls=":", label=f"Final = {yss4:.3f}")
ax.axhline(yss4 * 1.05, color="gray", ls="--", alpha=0.5, label="Faixa 5%")
ax.axhline(yss4 * 0.95, color="gray", ls="--", alpha=0.5)
ax.set_xlabel("Tempo (s)")
ax.set_ylabel("Amplitude")
ax.set_title("Q4 — Resposta ao degrau")
ax.legend()
salvar(fig, "q4_degrau.png")
tout4e, erro4 = tout4, 1.0 - yout4
fig, ax = plt.subplots(figsize=(7, 4))
ax.plot(tout4e, erro4, "r", lw=2)
ax.axhline(0.01, color="k", ls=":", label=r"$e_{ss}=1\%$")
ax.set_xlabel("Tempo (s)")
ax.set_ylabel("Erro")
ax.set_title("Q4 — Erro ao degrau")
ax.legend()
...
# --- Q5 ---
G5 = 4 / ((s + 1) * (s + 3) * (s + 4))
C5 = (2.04 * s**2 + 8.16 * s + 6.12) / s
L5, T5 = C5 * G5, ct.feedback(C5 * G5, 1)
plot_degrau(T5, "q5_degrau.png", r"Q5 — Resposta ao degrau ($\zeta=0{,}7$)", 6)
...
plot_degrau(T6, "q6_degrau.png", "Q6 — Resposta ao degrau (PI ZN)", 25)
...
info7 = ct.step_info(T7)
t7 = np.linspace(0, 5, 2500)
tout7, yout7 = ct.step_response(T7, t7)
fig, ax = plt.subplots(figsize=(7, 4))
ax.plot(tout7, yout7, "b", lw=2)
ax.axhline(1.0, color="k", ls=":", alpha=0.6)
ax.axhline(0.98, color="gray", ls="--", alpha=0.5, label="Faixa 2%")
ax.axhline(1.02, color="gray", ls="--", alpha=0.5)
ax.plot(info7["PeakTime"], info7["Peak"], "ro", ms=7,
        label=f"Pico: {info7['Overshoot']:.1f}%")
ax.set_xlabel("Tempo (s)")
ax.set_ylabel("Amplitude")
ax.set_title("Q7 — Resposta ao degrau (PI ajustado)")
ax.legend()
salvar(fig, "q7_degrau.png")
...
plot_degrau(T8, "q8_degrau.png", r"Q8 — Resposta ao degrau ($\zeta=0{,}43$)", 10)
